...
describe('Limiter', () => {
    ...
    it('invoke 한도를 초과하면 waitDraft를 호출해 다음 실행을 지연한다', async () => {
        ...
        const waitDraftSpy = jest.spyOn(limiter as any, 'waitDraft').mockResolvedValue(true as true);
        const waitSpy = jest.spyOn(limiter as any, 'waitMs').mockResolvedValue(true as true);
        const task = jest.fn();
